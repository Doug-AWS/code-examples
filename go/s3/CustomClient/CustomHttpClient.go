...
func GetObjectWithTimeout(sess *session.Session, bucket, object *string, duration int) (io.ReadCloser, error) {
	var body io.ReadCloser
	svc := s3.New(sess)

	// snippet-start:[s3.go.customHttpClient.get_object]
	ctx, cancelFn := context.WithTimeout(context.TODO(), time.Duration(duration)*time.Second)
	defer cancelFn()
...
func main() {
	...
	timeout := flag.Bool("t", false, "Whether to get the object within duration seconds timeout or use the default custom HTTP client")
	duration := flag.Int("d", 0, "The duration of the timeout")
	flag.Parse()
	...
	if *timeout && *duration < 0 {
		fmt.Println("You must set a timeout value greater than zero")
		return
	}
	...
	if *timeout {
		...
		body, err = GetObjectWithTimeout(sess, bucket, object, *duration)
		if err != nil {
